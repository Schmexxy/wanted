...
if SERVER then
   CreateConVar("wntd_crowbar_unlocks", "1", FCVAR_ARCHIVE)
   CreateConVar("wntd_crowbar_pushforce", "395", FCVAR_NOTIFY)
end
...
function SWEP:OpenEnt(hitEnt)
   ...
   if SERVER and GetConVar("wntd_crowbar_unlocks"):GetBool() then
...
function SWEP:SecondaryAttack()
   ...
   if tr.Hit and IsValid(tr.Entity) and tr.Entity:IsPlayer() and (self.Owner:EyePos() - tr.HitPos):Length() < 100 then
      ...
      if SERVER and (not ply:IsFrozen()) then
         local pushvel = tr.Normal * GetConVar("wntd_crowbar_pushforce"):GetFloat()
