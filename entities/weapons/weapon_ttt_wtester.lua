...
if CLIENT then
   CreateClientConVar("wntd_dna_scan_repeat", 1, true, true)
else
   function SWEP:GetRepeating()
      local ply = self.Owner
      return IsValid(ply) and ply:GetInfoNum("wntd_dna_scan_repeat", 1) == 1
   end
end
...
function SWEP:GatherObjectSample(ent)
   if ent:GetClass() == "wntd_c4" and ent:GetArmed() then
...
function SWEP:AddPlayerSample(corpse, killer)
   if #self.ItemSamples < self.MaxItemSamples then
      ...
      if not table.HasTable(self.ItemSamples, prnt) then
         ...
         hook.Call("WNTDFoundDNA", GAMEMODE, self.Owner, killer, corpse)
      end
      return true
   end
   return false
   ...
   if #self.ItemSamples < self.MaxItemSamples then
      ...
      for _, p in pairs(ent.fingerprints) do
         ...
         elseif table.HasTable(self.ItemSamples, prnt) then
            ...
            new = new + 1
            hook.Call("WNTDFoundDNA", GAMEMODE, self.Owner, p, ent)
         end
      end
      return new, old, own
   end
...
if SERVER then
   ...
   function SWEP:SendPrints(should_open)
      net.Start("wntd_ShowPrints", self.Owner)
   ...
   function SWEP:SendScan(pos)
      ...
      net.Start("wntd_ScanResult", self.Owner)
        net.WriteBit(clear)
        if not clear then
          net.WriteVector(pos)
        end
   ...
   local function GetScanTarget(sample)
      ...
      if IsValid(target.decoy) then
         target = target.decoy
      elseif not target:IsActivePlayer() then
...
if CLIENT then
   ...
   local function ShowPrintsPopup(item_prints, tester)
      ...
      delitem.DoClick = function()
                           if IsValid(ilist) and IsValid(ilist.SelectedPanel) then
                              ...
                              RunConsoleCommand("wntd_wtester_remove", idx)
                           end
                        end
      ...
      local bar = vgui.Create("WNTDProgressBar", mwrap)
      bar:SetSize(370, 35)
      bar:SetPos(0, 0)
      ...
      scan.DoClick = function(s)
                         if IsValid(ilist) then
                            ...
                            if IsValid(i) then
                               RunConsoleCommand("wntd_wtester_scan", i.key)
      ...
      dcheck:SetConVar("wntd_dna_scan_repeat")
   ...
   net.Receive("wntd_ShowPrints", RecvPrints)
   ...
   net.Receive("wntd_ScanResult", RecvScan)
   ...
   concommand.Add("wntd_wtester_scan", ScanPrint)
   ...
   concommand.Add("wntd_wtester_remove", RemoveSample)
