...
local ttt_telefrags = CreateConVar("wntd_teleport_telefrags", "0")
      ...
      for _, block in pairs(blockers) do
         if IsValid(block) then
            if block:IsPlayer() and block != ply then
               if block:IsActivePlayer() and block:Alive() then
...
local function DoTeleport(ply, teleport, weapon)
   if IsValid(ply) and ply:IsActivePlayer() and teleport then
      local fail = false
...
local function StartTeleport(ply, teleport, weapon)
   if (not IsValid(ply)) or (not ply:IsActivePlayer()) or (not teleport) then
      return end
...
function SWEP:TeleportRecall()
   ...
   if IsValid(ply) and ply:IsActivePlayer() then
