...
---- WNTD SPECIAL EQUIPMENT FIELDS

-- This must be set to one of the WEAPON_ types in WNTD weapons for weapon
...
if CLIENT then
   ...
   -- Open one of WNTD's icons with VTFEdit to see what kind of settings to use
...
---- MISC WNTD-SPECIFIC BEHAVIOUR CONFIGURATION

-- ALL weapons in WNTD must have weapon_tttbase as their SWEP.Base. It provides
-- some functions that WNTD expects, and you will get errors without them.
...
-- DO NOT set SWEP.WeaponID. Only the standard WNTD weapons can have it. Custom
...
local sparkle = CLIENT and CreateConVar("wntd_crazy_sparks", "0", FCVAR_ARCHIVE)

-- crosshair
if CLIENT then
   local sights_opacity = CreateConVar("wntd_ironsights_crosshair_opacity", "0.8", FCVAR_ARCHIVE)
   local crosshair_brightness = CreateConVar("wntd_crosshair_brightness", "1.0", FCVAR_ARCHIVE)
   local crosshair_size = CreateConVar("wntd_crosshair_size", "1.0", FCVAR_ARCHIVE)
   local disable_crosshair = CreateConVar("wntd_disable_crosshair", "0", FCVAR_ARCHIVE)
...
local ttt_lowered = CreateConVar("wntd_ironsights_lowered", "1", FCVAR_ARCHIVE)
