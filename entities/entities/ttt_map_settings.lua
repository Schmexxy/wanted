...
function ENT:Initialize()
   -- Backwards compatibility: WNTD maps compiled before the addition of the
   -- propspec setting may expect it to be off, so default it to off if a map
   -- settings entity exists (a reliable way of identifying a WNTD map)
...
function ENT:KeyValue(k, v)
   ...
   elseif k == "plymodel" and v != "" then -- can ignore if empty
      if util.IsValidModel(v) then
         ...
         Dev(2, "wntd_map_settings: set player model to be " .. v)
      else
         Dev(2, "wntd_map_settings: FAILED to set player model due to invalid path: " .. v)
      end
   elseif k == "propspec_named" then
      Dev(2, "wntd_map_settings: propspec possessing named props = " .. v)
...
function ENT:AcceptInput(name, activator, caller, data)
   if name == "SetPlayerModels" then
      ...
      if not mdlname then
         ErrorNoHalt("wntd_map_settings: Invalid parameter to SetPlayerModels input!\n")
         return false
      elseif not util.IsValidModel(mdlname) then
         ErrorNoHalt("wntd_map_settings: Invalid model given: " .. mdlname .. "\n")
         return false
      end

      GAMEMODE.force_plymodel = Model(mdlname)

      Dev(2, "wntd_map_settings: input set player model to be " .. mdlname)
