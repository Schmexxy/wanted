...
if SERVER then
   local ttt_hats_reclaim = CreateConVar("wntd_detective_hats_reclaim", "1")
   local ttt_hats_innocent = CreateConVar("wntd_detective_hats_reclaim_any", "0")
   ...
   local function TestHat(ply, cmd, args)
      if cvars.Bool("sv_cheats", 0) then
         local hat = ents.Create("wntd_hat_deerstalker")
   ...
   concommand.Add("wntd_debug_testhat", TestHat)
end
