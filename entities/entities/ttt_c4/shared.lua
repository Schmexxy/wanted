...
if SERVER then
   ...
   function ENT:ShowC4Config(ply)
      ...
      net.Start("wntd_C4Config")
         net.WriteUInt(self:EntIndex(), 16)
      net.Send(ply)
   end

   local function ReceiveC4Config(ply, cmd, args)
      if (not IsValid(ply)) or (not ply:IsActivePlayer()) or (not ply:Alive()) or #args != 2 then return end
   ...
   concommand.Add("wntd_c4_config", ReceiveC4Config)

   local function SendDisarmResult(ply, idx, result)
      net.Start("wntd_C4DisarmResult")
   ...
   local function ReceiveC4Disarm(ply, cmd, args)
      ...
      local bomb = ents.GetByIndex(idx)
      if IsValid(bomb) and bomb:GetClass() == "wntd_c4" and not bomb.DisarmCausedExplosion and bomb:GetArmed() then
         if bomb:GetPos():Distance(ply:GetPos()) > 256 then
            return
   ...
   concommand.Add("wntd_c4_disarm", ReceiveC4Disarm)


   local function ReceiveC4Pickup(ply, cmd, args)
      if (not IsValid(ply)) or (not ply:IsActivePlayer()) or (not ply:Alive()) or #args != 1 then return end
      local idx = tonumber(args[1])
   ...
   concommand.Add("wntd_c4_pickup", ReceiveC4Pickup)
   ...
   concommand.Add("wntd_c4_destroy", ReceiveC4Destroy)
end
