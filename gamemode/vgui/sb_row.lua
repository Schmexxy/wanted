...
function PANEL:Init()
	...
	self:AddColumn(
		GetTranslation("sb_deaths"), function(ply)
	...
	self:AddColumn(
		GetTranslation("sb_score"), function(ply)
...
function GM:WNTDScoreboardColorForPlayer(ply)
	if not IsValid(ply) then return namecolor.default end

	if ply:SteamID() == "STEAM_0:0:1963640" then
		return namecolor.dev
	elseif ply:IsAdmin() and GetGlobalBool("wntd_highlight_admins", true) then
		return namecolor.admin
	end
	return namecolor.default
end

function GM:WNTDScoreboardRowColorForPlayer(ply)
...
local function ColorForPlayer(ply)
	if IsValid(ply) then
		local c = hook.Call("WNTDScoreboardColorForPlayer", GAMEMODE, ply)

		-- verify that we got a proper color
		if c and type(c) == "table" and c.r and c.b and c.g and c.a then
			return c
		else
			ErrorNoHalt("WNTDScoreboardColorForPlayer hook returned something that isn't a color!\n")
		end
	end
	return namecolor.default
end
...
--   end
	...
	local c = hook.Call("WNTDScoreboardRowColorForPlayer", GAMEMODE, ply)
...
function PANEL:SetPlayer(ply)
	...
	if not self.info then
		...
		if g == GROUP_PLAYER and ply != LocalPlayer() then
			self.info = vgui.Create("WNTDScorePlayerInfoTags", self)
			self.info:SetPlayer(ply)

			self:InvalidateLayout()
		elseif g == GROUP_FOUND or g == GROUP_NOTFOUND then
			self.info = vgui.Create("WNTDScorePlayerInfoSearch", self)
			self.info:SetPlayer(ply)
			self:InvalidateLayout()
		end
	else
...
function PANEL:UpdatePlayerData()
	...
	if ScoreGroup(ply) != GROUP_PLAYER then
		ptag = nil
	end
...
vgui.Register( "WNTDScorePlayerRow", PANEL, "Button" )