...
function util.GetAlivePlayers()
   ...
   for k, p in pairs(player.GetAll()) do
      if IsValid(p) and p:Alive() and p:IsActivePlayer() then
...
local function DoBleed(ent)
   if not IsValid(ent) or (ent:IsPlayer() and (not ent:Alive() or not ent:IsActivePlayer())) then
      return
   end
...
function util.StartBleeding(ent, dmg, t)
   ...
   if ent:IsPlayer() and (not ent:Alive() or not ent:IsActivePlayer()) then
      return
   end
