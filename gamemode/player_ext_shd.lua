...
function plymeta:IsActivePlayer() return self:Team() == TEAM_PLAYER end
function plymeta:IsSpec() return self:Team() == TEAM_SPEC end

AccessorFunc(plymeta, "role", "Role", FORCE_NUMBER)

function plymeta:IsSpecial() return self:GetRole() != ROLE_PLAYER end

-- Player is alive and in an active round
function plymeta:IsActive()
   return self:IsActivePlayer() and GetRoundState() == ROUND_ACTIVE
end
...
if CLIENT then
   ...
   CreateConVar("wntd_show_gestures", "1", FCVAR_ARCHIVE)
   ...
   function plymeta:AnimPerformGesture(act, custom_runner)
      if GetConVarNumber("wntd_show_gestures") == 0 then return end
   ...
   net.Receive("WNTD_PerformGesture", function()
