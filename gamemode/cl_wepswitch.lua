...
WSWITCH.cv = {}
WSWITCH.cv.stay = CreateConVar("wntd_weaponswitcher_stay", "0", FCVAR_ARCHIVE)
WSWITCH.cv.fast = CreateConVar("wntd_weaponswitcher_fast", "0", FCVAR_ARCHIVE)
WSWITCH.cv.display = CreateConVar("wntd_weaponswitcher_displayfast", "0", FCVAR_ARCHIVE)
...
local col_active = {
   tip = {
      [ROLE_PLAYER]  = Color(55, 170, 50, 255)
   },
...
local col_dark = {
   tip = {
      [ROLE_PLAYER]  = Color(60, 160, 50, 155)
   },
...
function WSWITCH:DrawBarBg(x, y, w, h, col)
   ...
   local role = LocalPlayer():GetRole() or ROLE_PLAYER
...
concommand.Add("wntd_quickslot", QuickSlot)


local function SwitchToEquipment(ply, cmd, args)
   RunConsoleCommand("wntd_quickslot", tostring(7))
end
concommand.Add("wntd_equipswitch", SwitchToEquipment)
