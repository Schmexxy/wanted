...
function GM:Initialize()
	MsgN("Wanted Client initializing...")
	...
	RunConsoleCommand("wntd_spectate", GetConVar("wntd_spectator_mode"):GetInt())
end

function GM:InitPostEntity()
	MsgN("Wanted Client post-init...")
	...
	RunConsoleCommand("wntd_request_serverlang")
	RunConsoleCommand("wntd_request_rolelist")
end
...
local function RoundStateChange(o, n)
	if n == ROUND_PREP then
		...
		if GetConVar("wntd_spectator_mode"):GetBool() and IsValid(LocalPlayer())then
	...
	if n == ROUND_PREP then
		-- can enter PREP from any phase due to wntd_roundrestart
		hook.Call("WNTDPrepareRound", GAMEMODE)
	elseif (o == ROUND_PREP) and (n == ROUND_ACTIVE) then
		hook.Call("WNTDBeginRound", GAMEMODE)
	elseif (o == ROUND_ACTIVE) and (n == ROUND_POST) then
		hook.Call("WNTDEndRound", GAMEMODE)
	end
...
concommand.Add("wntd_print_playercount", function() print(GAMEMODE.StartingPlayers) end)

--- optional sound cues on round start and end
CreateConVar("wntd_cl_soundcues", "0", FCVAR_ARCHIVE)

local cues = {
	Sound("wntd/thump01e.mp3"),
	Sound("wntd/thump02e.mp3")
};
local function PlaySoundCue()
	if GetConVar("wntd_cl_soundcues"):GetBool() then
		surface.PlaySound(table.Random(cues))
	end
end

GM.WNTDBeginRound = PlaySoundCue
GM.WNTDEndRound = PlaySoundCue
...
net.Receive("WNTD_Role", ReceiveRole)
...
net.Receive("WNTD_RoleList", ReceiveRoleList)
...
net.Receive("WNTD_RoundState", ReceiveRoundState)

local function ReceiveAssignedPlayer()
	GAMEMODE.AssignedPlayer = net.ReadTable()
end
net.Receive("WNTD_AssignedPlayer", ReceiveAssignedPlayer)

local function ReceiveWonPlayers()
	GAMEMODE.WonPlayers = net.ReadTable()
	print(table.ToString(GAMEMODE.WonPlayers))
end
net.Receive("WNTD_WonPlayers", ReceiveWonPlayers)

local function ReceiveWonScore()
	GAMEMODE.WonScore = net.ReadUInt(3)
	print(GAMEMODE.WonScore)
end
net.Receive("WNTD_WonScore", ReceiveWonScore)
...
function GM:ClearClientState()
	...
	for _, p in pairs(player.GetAll()) do
		if IsValid(p) then
			p.sb_tag = nil
			p:SetRole(ROLE_PLAYER)
			p.search_result = nil
		end
	end

	RunConsoleCommand("wntd_mute_team_check", "0")

	if GAMEMODE.ForcedMouse then
		gui.EnableScreenClicker(false)
	end
end
net.Receive("WNTD_ClearClientState", GM.ClearClientState)
...
net.Receive("WNTD_PlayerSpawned", PlayerSpawn)

local function PlayerDeath()
end
net.Receive("WNTD_PlayerDied", PlayerDeath)
...
function CheckIdle()
	...
	if GetRoundState() == ROUND_ACTIVE and client:IsActivePlayer() and client:Alive() then
		...
		elseif CurTime() > (idle.t + idle_limit) then
			...
			timer.Simple(0.3, function()
										RunConsoleCommand("wntd_spectator_mode", 1)
										RunConsoleCommand("wntd_cl_idlepopup")
									end)
