...
CreateConVar("wntd_language", "auto", FCVAR_ARCHIVE)
...
function LANG.SetActiveLanguage(lang_name)
   ...
   if LANG.IsLanguage(lang_name) then
      ...
      if old_name != lang_name then
         hook.Call("WNTDLanguageChanged", GAMEMODE, old_name, lang_name)
      end
   else
...
function LANG.Init()
   local lang_name = GetConVarString("wntd_language")
...
cvars.AddChangeCallback("wntd_language", LanguageChanged)

local function ForceReload()
   LANG.SetActiveLanguage("english")
end
concommand.Add("wntd_reloadlang", ForceReload)
