...
local hl2_ammo_replace = {
   ...
   ["item_ammo_crate"] = "weapon_wntd_confgrenade",
   ["item_item_crate"] = "wntd_random_ammo"
};

-- Replace an ammo entity with the WNTD version
...
function ents.WNTD.RemoveRagdolls(player_only)
...
function ents.WNTD.ReplaceEntities()
   ReplaceAmmo()
   ReplaceWeapons()
   RemoveCrowbars()
   ents.WNTD.RemoveRagdolls()
end
...
function ents.WNTD.SetReplaceChecking(state)
   if state then
...
function ents.WNTD.FixParentedPreCleanup()
...
function ents.WNTD.FixParentedPostCleanup()
   for _, rcls in pairs(broken_parenting_ents) do
      for k,v in pairs(ents.FindByClass(rcls)) do
...
function ents.WNTD.TriggerRoundStateOutputs(r, param)
   r = r or GetRoundState()

   for _, ent in pairs(ents.FindByClass("wntd_map_settings")) do
      if IsValid(ent) then
...
-- Cache this, every wntd_random_weapon uses it in its Init
local SpawnableSWEPs = nil
function ents.WNTD.GetSpawnableSWEPs()
...
function ents.WNTD.GetSpawnableAmmo()
...
-- by wntd_weapon_spawn_max cvar) at randomly selected
...
local function PlaceWeaponsAtEnts(spots_classes)
   ...
   local spawnables = ents.WNTD.GetSpawnableSWEPs()
   
   local max = GetConVar( "wntd_weapon_spawn_count" ):GetInt()
   if max == 0 then 
      max = game.MaxPlayers()
...
-- act appropriately
function ents.WNTD.PlaceExtraWeapons()
   ...
   -- All current WNTD mappers use these, so if we find one we're good
...
local function RemoveWeaponEntities()
   ...
   for _, cls in pairs(ents.WNTD.GetSpawnableAmmo()) do
      for k, ent in pairs(ents.FindByClass(cls)) do
         ent:Remove()
      end
   end

   for _, sw in pairs(ents.WNTD.GetSpawnableSWEPs()) do
   ...
   ents.WNTD.RemoveRagdolls(false)
   RemoveCrowbars()
end
...
function ents.WNTD.CanImportEntities(map)
   if not tostring(map) then return false end
   if not GetConVar("wntd_use_weapon_spawn_scripts"):GetBool() then return false end
...
local function ImportSettings(map)
   if not ents.WNTD.CanImportEntities(map) then return end
...
local classremap = {
   wntd_playerspawn = "info_player_deathmatch"
};
...
function ents.WNTD.ProcessImportScript(map)
