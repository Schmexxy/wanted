...
for _, fname in pairs(files) do
   ...
   if string.Right(fname, 3) == "lua" then
      ...
      MsgN("Included WNTD language file: " .. fname)
   end
end
...
if SERVER then
   ...
   CreateConVar("wntd_lang_serverdefault", "english", FCVAR_ARCHIVE)

   local function ServerLangRequest(ply, cmd, args)
      if not IsValid(ply) then return end

      net.Start("WNTD_ServerLang")
         net.WriteString(GetConVarString("wntd_lang_serverdefault"))
      net.Send(ply)
   end
   concommand.Add("wntd_request_serverlang", ServerLangRequest)
   ...
   net.Receive("WNTD_LangMsg", RecvMsg)
   ...
   local function RecvServerLang()
      ...
      if LANG.Strings[lang_name] then
         if LANG.IsServerDefault(GetConVarString("wntd_language")) then
   ...
   net.Receive("WNTD_ServerLang", RecvServerLang)
end
